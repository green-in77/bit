...
public class Weight_Manager {
    public Scanner keyboard = new Scanner(System.in); // 입력받기 사용.
    public String str_name="기본값"; // 이름
    public String str_weight="기본값"; // 몸무게
    public String str_password="기본값"; // 비밀번호
    String[] txt_data=null; // 문자열배열
    File f; // 파일쓸꼬야
    File fd= new File("C://WeightMgData/"); //파일이 생성될곳
   
    public static void main(String[] args) {
    	Weight_Manager wm = new Weight_Manager(); // 몸무게 매니저 객체 생성
        int choiceInt;
        ...
        while (true) {
            ...
            try {
                wm.showMenu(); // 메뉴출력
                choiceInt = wm.keyboard.nextInt(); //선택한 번호 할당
                ...
                case 2: wm.searchWeight();break; // 찾기
                case 3: wm.changeWeight();break; // 몸무게바꾸기
                case 4: wm.changePassword();break; // 비밀번호 바꾸기
                case 5:
                    System.out.println("프로그램을 종료합니다.");
                    return;
                    // System.exit(0);
                }// switch문 닫기
            } catch (MenuChoiceException e) { // 1~5 이외의 숫자 입력시 
                e.showWrongChoice();
            ...
            } catch(Exception e){ // 입력받은 값이 숫자 이외의 것일경우.
    ...
    public void inputWeight() throws IOException{ 
        //boolean fileExist; //어디서썼지..?
       
        System.out.println("이름을 입력하여 주십시요.");
        System.out.print("이름 : "); 
        this.str_name = keyboard.nextLine(); //str_name 에 이름저장.
        System.out.println();
       //저장된 이름으로 파일객체 생성
        File f= new File("C://WeightMgData/"+this.str_name+".txt"); //기록할 파일객체 생성 // 덮어쓰기
        if( !f.exists()){      // 파일이 없으면 입력받기 ( 동일이름으로는 파일을 중복으로 만들 수 없음)
       
            System.out.println("몸무게를 입력하여 주십시요.");
            System.out.print("몸무게 : ");
            this.str_weight = keyboard.nextLine(); //str_weight 에 몸무게저장
            System.out.println();
            System.out.println("비밀번호를 입력하여 주십시요.");
            System.out.print("비밀번호 : ");
            this.str_password = keyboard.nextLine(); // str_password 에 비밀번호 저장
            System.out.println();
    ...
    public void searchWeight() throws IOException{
        ...
        // filecheck 실행해서 파일이 있으면 true 없으면 false 
        // 파일이 없으면 여기선 실행될 것이 없음..
       
        if (passcheck) { // 파일이 있으면 filecheck 에서 저장했던 이름과 몸무게 보여줌
            System.out.println("*****************************************************************");
    ...
    public void changeWeight() throws IOException{
     ...
     // 파일이 없으면 여기선 실행될 것이 없음..
        ...
        if (passcheck) { //파일이 있으면 가져온 데이터 보여주고 변경할 몸무게 입력
            System.out.println("*****************************************************************");
            ...
            bw.write(this.str_weight+":"); // 있는 파일에 덮어쓰기.
        ...
        showResult(); // true 인 경우에만 입력한 값 보여주기
    ...
    public void changePassword() throws IOException{
        ...
        if (passcheck) { // 파일이 있으면 가져온 정보 보여주고 변경할 패스워드 입력.
            System.out.println("*****************************************************************");
    ...
    public boolean fileCheck() throws IOException{  
        ...
        f= new File(fd.getAbsolutePath()+"/"+this.str_name+".txt"); //입력받은이름으로 파일경로 생성
        				// File fd= new File("C://WeightMgData/") //의 절대경로
        if(f.exists()){ //파일이 존재하면
            ...
            // 몸무게:패스워드 -> txt_data[0] 몸무게 , txt_data[1] 비밀번호
            ...
            for( int cnt = 3;cnt>0;cnt--){ 
                ...
                if (txt_data[1].equals(this.str_password)) {  // 1번방에 들은 비밀번호와 입력한 비밀번호 일치여부 확인
                    this.str_weight = txt_data[0]; // 비밀번호가 맞으면 str_weight 에 몸무게 저장
                    boo =  true; //상태체크 true
                    break;
                } else {
                   
                    if(cnt > 1) { // 2회차까지 알려주고 재입력
                    System.out.println("입력하신 비밀번호가 일치하지 않습니다.");
                    System.out.print((cnt-1) + "번의 기회가 남았습니다. 올바른 ");
                    }else{ // 마지막 회차에는 메뉴처음으로 돌아감.
        ...
        }else{//f.getName() 파일이 없으면
            System.out.println(str_name+"님 이(가) 존재하지 않습니다.");
            boo = false;
            }          
        return boo; // 파일이 있으면 true 없으면 false 
    }
