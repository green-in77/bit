...
public class BoardDao {
	...
	public int writeok(Board boarddata) {
		...
		try{
			...
			//1. 원본글 : refer 생성? , step 0, depth 0,
			//2. 답변글 : refer 생성? , step +1, depth +1
			
			int refermax = getMaxRefet();
	...
	private int getMaxRefet() {
		...
		try {
			conn = ds.getConnection(); //conn 에서 연결 객체 빌려옴..
	...
	//게시물 목록보기
	public List<Board> getList(int cpage, int pagesize) {
		/*
		  [1][2][3][4][5]다음
		  [이전][6][7][8][9][10][다음]
		  ...
		  totaldata > 54건
		  pagesize = 5
		    규칙 > totalpagecount = 11(전체 페이지의 개수)
		  
		  int cpage : currentpage 현재 페이지의 번호 >> 1page, 2page...
		  
		    현재 데이터가 100건
		  cpage 1 , pagesize 5 > start(시작글번호) 1 ~ end(끝글번호) 5
		  cpage 2 , pagesize 5 > start(시작글번호) 6 ~ end(끝글번호) 10
		  
		  cpage 11 , pagesize 5 > start(시작글번호) 51 ~ end(끝글번호) 55
		  5개씩 묶어서 11번째 묶음을 보여주기* 아래 2개의 계층형 페이징처리 쿼리 테스트 하기 
...
//				String homepage = rs.getString("hompage");
...
//				board = new Board(idx, writer, pwd, subject, content, writedate, readnum, filename, filesize, homepage, email, refer, depth, step);
	...
	public int deleteOk(String idx, String pwd) {
		...
		//계층형 게시판 : 답글?댓글?
		/*
		 	1. 원본글(답글이 없는 경우) : 그냥 삭제
		 	2. 원본글(답글이 있는 경우)
		 	case 1: 답변글 전부 삭제 (같은 refer delete)
		 	case 2: (네이버)원본글만 삭제 -> 나머지처리(텍스트 형태(원본글 삭제표시)) 같은 refer 중에 depth, step 0인 값이 있는지.
		 	case 3: 게시물 남겨두고 내용이 삭제됨을 알려줌.(게시판 설계 (삭제여부 컬럼으로 생성해야함... 기본값 :1, 삭제하면  :0)
		 	case 4: 삭제를 못하게 한다.. -> 답글이 있으면.. refer count 가 1이상이면...
		 */
		//case 1
		Connection conn = null;
		PreparedStatement pstmt = null;
		...
		try {
			...
			//댓글에 FK 걸려있기때문에 댓글먼저 지워야한다.... cascade 옵션
			...
			pstmt = conn.prepareStatement(sql_pwd);
			pstmt.setString(1, idx);
			rs = pstmt.executeQuery(); //pwd 검증
				
			if(rs.next()) { //삭제글은 존재.
				if( pwd.equals(rs.getString("pwd"))) { //글의 비번과 입력한 비번이 맞는지
					//삭제처리 ★★★★ 트랜잭션 ★★★★ 
					//둘다 처리 되거나, 둘다 실패하거나
					//두개를 하나의 논리적 단위로 묶는다..
					//JDBC : auto commit
