...
<%
	...
	// RealPath : 실배포경로 - 톰캣을 이클립스에 내장하고 배포했을때 경로
	//게시판 만들기(파일 업로드 시... 아래 코드 활용)
	String realContextpath = application.getRealPath(request.getContextPath());
	out.print("realContextpath : " + realContextpath+"<br>");
	//C:\SmartWeb\WebJSP\JSPLab\.metadata\.plugins\org.eclipse.wst.server.core\tmp0\wtpwebapps\WebJSP\WebJSP
	
	String realFilepath = application.getRealPath(resourcepath);
	out.print("realFilepath : " + realFilepath+"<br>");
	//C:\SmartWeb\WebJSP\JSPLab\.metadata\.plugins\org.eclipse.wst.server.core\tmp0\wtpwebapps\WebJSP\download\note.txt
	
	out.print("request.getRequestURL() : " + request.getRequestURL()+"<br>"); // http://localhost:8090/WebJSP/Ex11_WebApp_RealPath.jsp
	out.print("request.getRequestURI() : " + request.getRequestURI()+"<br>"); // /WebJSP/Ex11_WebApp_RealPath.jsp
	out.print("request.getContextPath() : " + request.getContextPath()+"<br>"); // /WebJSP
	out.print("request.getServletPath() : " + request.getServletPath()+"<br>"); // /Ex11_WebApp_RealPath.jsp
%>
</body>
</html>