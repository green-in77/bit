...
public class Ex01_Oracle_Connection {
	public static void main(String[] args) throws Exception {
		...
		Connection conn = DriverManager.getConnection("jdbc:oracle:thin:@localhost:1521:xe","bituser","1004");
														//연결DB의 Port, id, pw
		//getConnection 내부에서 new Connection 생성 -> connection 인터페이스를 부모로 두고있기때문에... 오라클,mysql 등등 모두 커넥션으로 받음/ 생성된 객체를 리턴.
		...
		ResultSet rs = stmt.executeQuery(sql); // DB 서버에 생성된 결과집합(memory) 접근해서 Data read
		
		//4. 처리(조회)
		while(rs.next()) { //next = select 한 결과집합이 있니???
			System.out.println(rs.getInt("empno") + "/" + rs.getString("ename")+"/"+rs.getInt("sal"));
			//							  1번째컬럼						2번째컬럼				  3번째컬럼
			//System.out.println(rs.getInt("1") + "/" + rs.getString("2")+"/"+rs.getInt("3")); 가능
		}
		
		//5. 연결해제
		rs.close();
		conn.close();
		System.out.println("연결끊어졌니?? " + conn.isClosed() + " (True >> 연결끊어짐)"); //너 연결 끊어졌니??
