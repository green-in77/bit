...
END;
--이미 만들어 둔 프로시저 이다 : java System.out.println
...
DECLARE --선언
  vno number(4); --변수명 타입 ; 세미콜론!!붙여야 한다. 엄청 예민함 없으면 실행불가
  vname varchar2(20);
BEGIN --변수의 scope = begin~end 
...
BEGIN
  ...
  commit; --commit, rollback 지정 해줘야 함.
END;
...
increment by 1
start with 8000 --시작값 8000
maxvalue 9999 --최대 9999까지
nocycle --미리 값을 가져오지 않음..?
nocache;

DECLARE
  v_empno emp.empno%TYPE;
BEGIN --여러개의 query문이 들어가도 된다.
...
create table empdml
as
    select * from emp where 1=2; --1=2 : 구조만 복사

--여기까지 변수 선언 , 타입 , 값 할당
--------------------------------------------------------------------------------
--pl-sql 제어문
DECLARE
  vempno emp.empno%TYPE; --emp테이블의 empno 타입을 가져온다.
...
BEGIN
    ...
    from emp
    where empno=7788;
    --제어문 if(조건문){실행문} 모든 구문에 세미콜론을 꼭 붙여야한다
    IF(vdeptno = 10) THEN vname := 'ACC'; -- if(vdeptno==10) { vname = "ACC"} : then 이 실행블럭
        ELSIF(vdeptno=20) THEN vname := 'IT'; --else if 아니다.. elsif..
...
--sursor는 메모리에 올려서 작업하기 때문에 성능이 좋아야 한다. 
...
BEGIN
  ...
  LOOP
      v_sal_total := v_sal_total + emp_record.sal; --total 변수에 누적
...
as
  select * from emp where 1=2; --emp 테이블 구조만 복사(스키마 만 복사)
